...
(() => {
  ...
  function makePreview(context, activities = ['memorize', 'recall', 'spell']) {
    ...
    return {
      version: VERSION,
      page: location.href,아setId: context.setId,
  ...
  async function prepareTest(context, options = {}) {
    ...
    try {
      ...
      return {
        ...
        submitUrl:
          context.setType === 5
            ? '/ClassTest/submittest'
            : '/ClassTest/submit_gpt',
        payload: formDataFromDetachedForm(form),
  ...
  async function submitTest(options = {}) {
    ...
    let data;
    try {
      data = await fetchJson(prepared.submitUrl, {
        method: 'POST',
